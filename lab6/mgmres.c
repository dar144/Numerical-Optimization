...
/*
  ...
  Input:
    ...
    int NZ_NUM, the number of nonzeros.

    int IA[N+1], JA[NZ_NUM], the row and column indices
    of the matrix values.  The row vector has been compressed.

    double A[NZ_NUM], the matrix values.

    int UA[N], the index of the diagonal element of each row.

  Output:

    double L[NZ_NUM], the ILU factorization of A.
*/
{
  int *iw;
  int i;
...
/*
  ...
  Input:
    ...
    int N, the order of the linear system.

    int NZ_NUM, the number of nonzero matrix values.
    ...
    double X[N]; an approximation to the solution.

    double RHS[N], the right hand side of the linear system.

    int ITR_MAX, the maximum number of (outer) iterations to take.

    int MR, the maximum number of (inner) iterations to take.
    MR must be less than N.

    double TOL_ABS, an absolute tolerance applied to the
    current residual.

    double TOL_REL, a relative tolerance comparing the
    current residual to the initial residual.

  Output:

    double X[N]: an improved approximate solution.
...
*/
  l = ( double * ) malloc ( nz_num * sizeof ( double ) );
  r = ( double * ) malloc ( n * sizeof ( double ) );
...
/*
  ...
  Output:
    ...
    double R8VEC_DOT, the dot product of the vectors.
*/
{
  int i;
  double value;
